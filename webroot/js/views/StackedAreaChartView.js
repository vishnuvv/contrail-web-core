...
//                    .on('resize', resizeFunction);
        ...
        renderChart: function (selector, viewConfig, chartViewModel) {
            ...
            if ($(selector).find('.stacked-area-chart-container').find("svg") != null &&
                    $(selector).find('.stacked-area-chart-container').find("svg").length > 0) {
                $(selector).find('.stacked-area-chart-container').empty();
            } else {
                $(selector).html(chartTemplate);
                if (widgetConfig !== null) {
                    this.renderView4Config($(selector).find('.stacked-area-chart-container'), chartViewModel, widgetConfig, null, null, null);
                }
            }
