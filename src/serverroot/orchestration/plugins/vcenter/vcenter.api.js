/*
    ...
    if (config.vcenter.sdkPath) {
        serverPort = config.vcenter.sdkPath;
    }
    if (config.vcenter.wsdl) {
        serverPort = config.vcenter.wsdl;
    }
}

var vCenterSoapServer = rest.getSOAPApiServer({apiName:global.label.OPS_API_SERVER,
...
function populatevCenterParams(appData) {
    return new Promise(function(resolve,reject) {
        if(dataCenterName != null && vSwitchName != null)
            ...
            getIdByMobName(appData,'Datacenter','kiran_dc').done(function(response) {
                dataCenterName = response;
                getIdByMobName(appData,'DistributedVirtualSwitch','kiran_dvswitch').done(function(response) {
...
function createPortGroup(userData,appData,callback) {
    ...
    console.info('portGroupData',portGroupData);
    return new Promise(function(resolve,reject)  {
