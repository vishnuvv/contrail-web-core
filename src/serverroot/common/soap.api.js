...
{
    console.log("Getting sel as:", self);
    self.name = apiName;
    return {
        name:apiName,
...
SoapAPIServer.prototype.makeCall = function (soapClient, userData, callback)
{
    ...
    console.log("Getting userData as:", JSON.stringify(userData));
    soapClient.call(userData)
        .done(
            function(res) {
                console.log("getting resp as:", res);
                if ((null != res) && (null != res['data']) && 
                    (null != res['data']['Fault']) && 
                    (null != res['data']['Fault']['faultstring'])) {
                console.log("getting MESSAGE AS:", JSON.stringify(res['data']));
                    var error =
