...
function readAndProcessPkgXMLFiles (pkgDir, pkgName, callback)
{
    var fileListArr = [];
    ...
    writeToPkgFile(pkgDir, true, str, function() {
        dir.readFiles(pkgDir, {
            ...
            if ('application/xml' != mime.lookup(filename)) {
                ...
                autoGenFile = autoGenFile.replace(pkgDir, "");
...
function deleteAllAutoGenFiles (callback)
{
    ...
    for (key in config.featurePkg) {
        if (false == fs.existsSync(config.featurePkg[key]['path'])) {
            continue;
        }
    ...
    dir.readFiles(pkgDir, {
        ...
        function(err, content, next) {
            ...
            if ((null != err) || (null == files) || (!files.length)) {
                console.log("dir.readFiles error for Dir:" + pkgDir + ":" +
                            err);
                callback(err);
                return;
            }
